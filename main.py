...
router = NatsRouter(f"nats://{config.nats_host}:{config.nats_port}")
...
async def create_face_detection_job(request: FaceOperationJobFile):
    response: NatsMessage = await router.broker.request(
        request, "cpu-tasks", timeout=30
    )
...
if __name__ == "__main__":
    uvicorn.run(
        "main:app", host=config.service_host, port=config.service_port, reload=True
    )
