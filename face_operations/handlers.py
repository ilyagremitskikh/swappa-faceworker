    ...
    def handle(self, context: ProcessingContext) -> None:
        ...
        try:
            ...
            if self.field_name:
                ...
                if hasattr(context, "source_image_url") and not hasattr(context, "target_path"):
                    url_field = "source_image_url"
                    path_field = "source_path"
                elif hasattr(context, "target_image_url") and not hasattr(context, "source_path"):
                    url_field = "target_image_url"
                    path_field = "target_path"
                else:
                    context.error = Exception("Неизвестный тип контекста или необходимо указать field_name")
                    return
            else:
...
class DetectFacesHandler(Handler):
    ...
    def handle(self, context: FaceVerificationContext) -> None:
        ...
        try:
            ...
            # Вызов детектора
            bounding_boxes, face_scores, face_landmarks_5 = detect_faces(context.image_data)
            
            # Применяем собственный NMS для удаления дублей
            unique_indices = self._apply_nms(bounding_boxes, face_scores, iou_threshold=0.5)
    ...
    def _initialize_face_detector_state(self):
        ...
        if not state_manager.get_item('config_path'):
            config_path = str(Path(Path(__file__).parent.parent, '.assets', 'configs', 'default.ini'))
...
def DownloadSourceImageHandler(temp_dir: str) -> DownloadImageHandler:
    """Создает обработчик для скачивания исходного изображения."""
    return DownloadImageHandler(temp_dir, prefix="source", field_name="source")


def DownloadTargetImageHandler(temp_dir: str) -> DownloadImageHandler:
    """Создает обработчик для скачивания целевого изображения."""
    return DownloadImageHandler(temp_dir, prefix="target", field_name="target")
